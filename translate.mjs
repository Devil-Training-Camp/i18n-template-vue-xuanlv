...
async function transform({ from, to, outputPath }) {
  ...
  for (let index = 0, keys = Object.keys(originLocales); index < keys.length; index += 1) {
    ...
    // 已翻译则跳过
    if (targetLocales[key]) {
      continue;
    }
    // 转换文本中的 `{}` 变量为数字，避免被翻译替换
    const variables = extractTextVariable(value);
    taskQueue.push({ key, value: fillVariableToNumber(value, variables), variables });
    // 队列已满时翻译
  ...
  async function batchTransition() {
    const res = await volcano({
      TextList: taskQueue.map((n) => n.value),
      TargetLanguage: to,
      SourceLanguage: 'zh',
    });
...
function fillVariableToNumber(text, variables) {
  return variables.reduce((prev, curr, index) => prev.replace(curr, `{${index}}`), text);
}
...
function restoreVariable(text, variables) {
  return variables.reduce((prev, curr, index) => prev.replace(`{${index}}`, curr), text);
}
